...
const listMessages = catchAsync(async (req, res) => {
  const conversation = await nodeCacheService.getConversation(req.query.conversation_id);
  let user1 = null;
  let user2 = null;
  if (!conversation) {
    return res.status(httpStatus.BAD_REQUEST).json(responseMessage('Invalid conversation', httpStatus.BAD_REQUEST));
  } else {
