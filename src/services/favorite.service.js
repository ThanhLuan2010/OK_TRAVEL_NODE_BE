...
const list = async (account_id) => {
  try {
    const sql1 = sequelize.query(
      `SELECT * FROM favorite_service_entity
...
// if favorite -> not; if not -> favorite
// return 0 -> error
// return 1 -> favorite
// return 2 -> no favorite
// return 3 -> invalid service
const action = async (account_id, objData) => {
  try {
    let data = await sequelize.query(
      `SELECT *
        FROM favorite_service_entity
    ...
    if (data && data.length > 0) {
      ...
      await sequelize.query(
        `DELETE FROM favorite_service_entity
        WHERE account_id = :account_id AND type = :type  AND ref_id = :ref_id`,
        {
          replacements: {
            account_id,
    ...
    if (data && data.length > 0) {
      await sequelize.query(
        `INSERT INTO favorite_service_entity
    ...
    logger.error(`ERR action favorite: ${e.message}`);
    return 0;
  }
};
